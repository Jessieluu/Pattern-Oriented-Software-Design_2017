...
TEST(Shell_Excep, exception_OnlyVariable) {
...
TEST(Shell_Excep, exceptionMissingPeriodToken) {
  Scanner s("X=1");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Shell_Excep, exception_UnexpectedDisjTokenBeforePeriod) {
  Scanner s("X;.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Shell_Excep, exception_UnexpectedConjTokenBeforePeriod1) {
  Scanner s("X,.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Shell_Excep, exception_UnexpectedConjTokenBeforePeriod2) {
  Scanner s("X=1,.");
  Parser p(s);
  try {
    p.buildExpression();
...
TEST(Shell_Excep, exception_UnbalancedOperator1) {
...
TEST(Shell_Excep, exception_UnbalancedOperator2) {
